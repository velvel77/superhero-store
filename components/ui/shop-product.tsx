...
export default function ShopProduct({ product, relatedProducts }: Props) {
  ...
  const stats = [
    { label: 'Power', value: power * 10, color: 'bg-secondary-500' },
    { label: 'Durability', value: durability * 10, color: 'bg-primary-500' },
    { label: 'Special', value: special * 10, color: 'bg-rarity-epic' },
  ];
  ...
  function addToCart() {
    ...
    const existingIndex = cart.findIndex(
      (item: { id: number; type: string }) => item.id === product.id && item.type === 'product',
    );
        ...
        <div className="flex flex-col">
          <main className="flex my-4 gap-4">
            ...
            <div className="relative flex-1">
              ...
              <span
                className={`absolute top-2 right-2 px-2 rounded-sm ${
                  product.stock > 0 ? 'text-rarity-uncommon bg-black' : 'text-basic-100 bg-secondary-500'
                }`}
              >
            ...
            <section className="flex-1">
              <div>
                <h2 className="text-shadow-md text-shadow-secondary-500 italic font-bold text-3xl">{product.name}</h2>
              ...
              <div>
                ...
                <section className="p-4 bg-pattern-benday my-4 rounded-sm border border-basic-400">
                  <div className="flex items-center gap-2 pb-2">
                    ...
                    <span className="italic font-bold uppercase text-xs">Gear details</span>
                  </div>
                ...
                <div className="flex py-4 gap-4">
                  ...
                  <button
                    ...
                    className={`size-16 border-2 hover:border-secondary-500 bg-effect-dark ${
                      favorite ? 'border-effect-red text-secondary-500' : 'border-basic-400 text-basic-400'
                ...
                {added ? <p className="text-[.7rem] font-bold uppercase text-rarity-uncommon">Added to cart</p> : null}
              </div>
              ...
              <section className="flex gap-6 text-[.7rem] *:policy">
                ...
                <div>
                  ...
                  <div className="flex flex-col">
                    <span className="italic font-bold uppercase">Certified gear</span>
          ...
          <section className="py-16">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {relatedProducts.map((related) => (
                <Link href={`/products/${related.id}`} className="relative block" key={related.id}>
                  <div className="relative group">
                    <div className="relative aspect-4/5 w-full overflow-hidden border-2 border-basic-400/20 bg-basic-800 group-hover:border-secondary-500">
                      ...
                      <Image
                        ...
                        src={related.image_url ? related.image_url : placeholder}
                        alt={related.name}
                        fill
                  ...
                  <div className="flex min-h-18 flex-col p-2">
                    <span className="text-secondary-500 uppercase text-[.5rem] tracking-wider">{related.category}</span>
                    <span className="italic uppercase font-bold text-[.7rem]">{related.name}</span>
                    <span className="text-primary-500">${related.price}</span>
                  </div>
                </Link>
              ))}
            </div>
