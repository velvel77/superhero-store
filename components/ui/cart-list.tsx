    ...
    if (items.length === 0) {
        return (
            <div>
                ...
                <span>Find what you need<Link href="http://localhost:3000/shop/"> in our shop!</Link></span>
            </div>
        )
    }
    return (
        <div>
            {items.map(item => (
                <div key={`${item.type}-${item.id}`}>
                    ...
                    <div>
                        <button onClick={() => decreaseQuantity(item.id, item.type)}>Minus</button>
                        <span>{item.quantity}</span>
                        <button onClick={() => increaseQuantity(item.id, item.type)}>Add</button>
                    </div>
