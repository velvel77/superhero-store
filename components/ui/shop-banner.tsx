    ...
    <section className="relative flex items-center border-b-2 border-white h-140">
      ...
      <div className="flex flex-col p-4 gap-4 w-full max-w-260 mx-auto z-10">
        ...
        <h1 className="uppercase text-white text-6xl font-bold italic flex flex-col px-2 gap-3">
          <span className="text-shadow-md text-shadow-(color:--color-effect-red)">Unleash your</span>{' '}
          <span className="text-(--color-effect-yellow)">inner legend</span>
        </h1>
        <div className="relative flex items-center pl-6">
          <span className="absolute left-4 h-0.5 w-20 bg-(--color-effect-red)"></span>
        ...
        <div className="text-white italic font-bold flex gap-2 px-4">
          ...
          <button className="p-4 uppercase bg-(--color-effect-gray) border-2 border-white catalog-hover">
            View Catalog
          </button>
        </div>
        ...
        <div className="flex gap-4 p-4">
          {stats.map((stat) => (
            <div key={stat.title}>
              <span className="text-(--color-effect-yellow) font-bold pr-2 italic">{stat.number}</span>
              <small className="text-gray-500 text-[.6rem] uppercase">{stat.title}</small>
            </div>
          ))}
        </div>
      </div>
